...
# 3, 4, 5 or more capitals letters. The 1st letter of a code is the capital letter of the book category. In 
# the bookseller's stocklist each code c is followed by a space and by a positive integer n (int n >= 0) 
...
# corresponding to "BKWRK" and "BTSQZ", 50 corresponding to "CDXEF" and 0 to category 'W' since there are no 
...
def stockList(listOfArt, listOfCat)
  ...
  listOfCat.map do |category| 
    "(#{category} : #{listOfArt.group_by { |group| group[0] }[category].to_a.map { |i| i.match(/\d+$/)[0].to_i }.sum })"
  end
  .join ' - '
end