...
# In this kata you should complete a function that take in an integer that correspond to n, and an history with the 
# following format:
...
# and that should return the nth executed command line, for example with n=4 and the above history it should return 
# touch me. If user ask for a n without any know entry for exampl n=12 here, the function should return !12: event not 
# found.

# Note: Lot of the command line comes form some geeky t-shirt and form this excellent page.

def bang_start_string(s, history)
  output = history.split('  ').reverse.map(&:strip).find { |comm| comm.start_with?(s) }
