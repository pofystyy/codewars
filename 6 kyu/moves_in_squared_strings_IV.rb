...
def diag_2_sym(strng)
  strng.split.map(&:chars).transpose.map(&:join).join("\n").reverse
end

def rot_90_counter(strng)
  diag_2_sym(strng).split.map(&:reverse).join("\n")
end

def selfie_diag2_counterclock(strng)
  diag = diag_2_sym(strng).split
  rot  = rot_90_counter(strng).split
  strng.split.zip(diag, rot).map { |i| i.join('|') }.join("\n")
end
