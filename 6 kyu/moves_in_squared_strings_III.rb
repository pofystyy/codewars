...
def diag_1_sym(strng)
  strng.split.map(&:split).map { |i| i[0].chars }.transpose.map(&:join).join("\n")
end

def rot_90_clock(strng)
  diag_1_sym(strng).split.map(&:reverse).join("\n")
end

def selfie_and_diag1(strng)
  strng.split.zip(diag_1_sym(strng).split).map { |i| i.join('|') }.join("\n")
end
