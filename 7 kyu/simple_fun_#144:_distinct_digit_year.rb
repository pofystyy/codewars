...
#     the minimum year number that is strictly larger than the input number year and all it's digits are 
#     distinct.

def distinct_digit_year(year)
  year = year + 1
  return year if year.digits.map { |i| p year.digits.count(i) == 1 }.all?
  distinct_digit_year(year)
end